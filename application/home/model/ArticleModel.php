...
class ArticleModel extends Model
{
     ...
     * 获取滑动新闻（含有图片）信息
     */
    public function getSlideNewsList($map,$limits=null)
    {
        return $this->where($map)->where(['photo'=>['neq','']])->order('isTop desc,sortnum desc, create_time desc,id desc')->limit($limits)->select();
    }
     ...
     * 分类多图
     */
    public function getOneCatePic($map)
    {
        return Db::name('pic_catelist')->where($map)->limit(1)->order('sortnum asc')->find();
    }
