...
class Category extends Base
{
    ...
    public function index()
    {
        ...
        if ($childCnt == 0||$cate_arr[1]['id'] == 14||$navCur == 10) {
            $cateCur = $cate;           //当前id
        }
        elseif($navCur == 42){
...
//        dump($third_id);
        ...
        if ($cateCur['info_state'] == 0 || $cateCur['info_state'] == 1) {
            // $cateParam = $cateCur['cateParam'];
            $lists = $articleModel->getOneArticle($map_list);
        } else if ($cateCur['info_state'] == 2 ||$cateCur['info_state'] == 3 ||$cateCur['info_state'] == 4 || $cateCur['info_state'] == 5 || $cateCur['info_state'] == 6 || $cateCur['info_state'] == 7) {
...
//        dump($subCur);
         ...
         * 相关推荐
         * */
        $about_lists = Db::name('article')->where(['cate_id'=>$info['cate_id']])->where(['id'=> ['neq',$info['id']]])->select();
        $this->assign('about_lists', $about_lists);
        /*右侧热文推荐*/
        $rightnews_lists = Db::name('article')->where(['cate_id'=>$info['cate_id'],'status'=>1])->order('isTop desc,sortnum desc')->limit(10)->select();
        $this->assign('rightnews_lists', $rightnews_lists);
